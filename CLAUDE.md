...
Ad hoc bot-logic regression scripts (not a test framework) live in `tools/botsim/tests/` (e.g. `repro_q_spades.js`, `repro_unsafe_lead.js`) — run directly with `node` to reproduce specific bot decision bugs against a hand-built context.
